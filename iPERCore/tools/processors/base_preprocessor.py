...
class BaseProcessor(metaclass=abc.ABCMeta):
    ...
    def execute(self, processed_info: ProcessInfo,
        ...
        # 4. find the front face images
        if find_front and not processed_info["has_find_front"]:
            print(f"\t1.4 Preprocessing, running {self.__class__.__name__} to find {num_candidate} "
                  f"candidates front images in {out_img_dir} ...")
            self._execute_post_find_front(processed_info, num_candidate, render_size)
            print(f"\t1.4 Preprocessing, finish find the front images ....")

        processed_info.serialize()

        # 5. run human parser
        if parser and not processed_info["has_run_parser"]:
            print(f"\t1.5 Preprocessing, running {self.__class__.__name__} to run human matting in "
                  f"{processed_info['out_parse_dir']} ... ")
            self._execute_post_parser(processed_info)
            print(f"\t1.5 Preprocessing, finish run human matting.")
    ...
    def _execute_crop_img(self, processed_info: ProcessInfo,
        ...
        if num_workers > 0:
            num_images = len(all_img_names)
            src_paths = [os.path.join(src_img_dir, img_name) for img_name in all_img_names]
            out_paths = [os.path.join(out_img_dir, img_name) for img_name in all_img_names]
            ...
            for i, image_name in enumerate(tqdm(all_img_names)):
    ...
    def _execute_detector(self, processed_info: ProcessInfo, factor: float):
        ...
        if factor == 0:
            min_size = min(orig_shape[1], orig_shape[0])
            cx = orig_shape[1] // 2
            cy = orig_shape[0] // 2
            half = min_size // 2
