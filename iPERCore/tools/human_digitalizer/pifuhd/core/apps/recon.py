...
from lib.data import EvalWPoseDataset, EvalDataset, EvalCroppedDataset
...
def recon(opt, use_cropped=False, use_rect=False):
    # load checkpoints
    ...
    if use_cropped:
        test_dataset = EvalCroppedDataset(opt)
    else:
        if use_rect:
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(start_id, end_id)):
            ...
            save_path = f"{opt.results_path}/{test_data['name']}.obj"

            gen_mesh(opt.resolution, netMR, cuda, test_data, save_path, components=opt.use_compose)


def reconWrapper(args=None, use_cropped=False, use_rect=False):
    opt = parser.parse(args)
    recon(opt, use_cropped=use_cropped, use_rect=use_rect)
