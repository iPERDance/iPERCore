...
class EvalWPoseDataset(Dataset):
    ...
    def __init__(self, opt, projection='orthogonal'):
        ...
        self.img_files = sorted([os.path.join(self.root, f) for f in os.listdir(self.root) if
                                 f.split('.')[-1] in ['png', 'jpeg', 'jpg', 'PNG', 'JPG', 'JPEG'] and os.path.exists(
                                     os.path.join(self.root, f.replace('.%s' % (f.split('.')[-1]), '_keypoints.json')))])
