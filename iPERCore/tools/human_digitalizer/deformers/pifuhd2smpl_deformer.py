...
# @See     : tools/tests/pifu2smpl_deformer_test.py
...
class PifuHD2SMPLRegister(object):
    def __init__(self, total_iters=600, smpl_model="assets/checkpoints/pose3d/smpl_model.pkl",
        ...
        self.smpl_verts_weights[np.array(part_info["08_left_hand"]["vertex"] + part_info["09_right_hand"]["vertex"])] *= self.w_hand
        self.smpl_verts_weights[np.array(part_info["06_left_foot"]["vertex"] + part_info["07_right_foot"]["vertex"])] *= self.w_foot
    ...
    def fit_pifuhd(self, cam, pose, shape, pifuhd_obj_path, verbose=False):
        ...
        for i in bar:
            ...
            recons_loss = self.compute_recons_loss(new_smpl_mesh.verts_packed(), self.smpl_faces, sampled_pifu_verts) * self.w_recons
...
class PifuHD2SMPLDeformer(object):
    def __init__(self,
                 total_iters=1000, smpl_model="assets/checkpoints/pose3d/smpl_model.pkl",
                 facial_path="assets/checkpoints/pose3d/front_facial.json",
                 fit_pifuhd_path="assets/checkpoints/pose3d/mapper_for_fit_pifuhd.txt",
                 ckpt_path="./assets/checkpoints/pose3d/pifuhd.pt", temp_dir="./assets/temp",
                 device=torch.device("cuda:0")):
        self.pifuhd_runner = PifuHDRunner(ckpt_path=ckpt_path, temp_dir=temp_dir, )

        self.pifuhd_register = PifuHD2SMPLRegister(
            total_iters=total_iters, smpl_model=smpl_model,
            facial_path=facial_path,
            fit_pifuhd_path=fit_pifuhd_path,
            device=device
        )

    def deform(self, img_path, output_dir, init_smpls):
        """

        Args:
            img_path (str):
            output_dir (str):
            init_smpls (np.ndarray): (1, 85)
        ...
        cams = init_smpls[:, 0:3]
        poses = init_smpls[:, 3:-10]
        shape = init_smpls[:, -10:]

        pifu_obj_path = self.pifuhd_runner.recons(img_path, output_dir)
        new_pose, offsets, reg_verts = self.pifuhd_register.fit_pifuhd(
            cams, poses, shape, pifu_obj_path, verbose=True)

        offsets = offsets.detach().cpu().numpy()
        reg_verts = reg_verts.detach().cpu().numpy()

        reg_obj_path = os.path.join(output_dir, f"register_{os.path.split(pifu_obj_path)[-1]}")
        reg_verts[:, 1:] *= -1
        self.pifuhd_register.export_mesh(reg_verts, reg_obj_path)

        return new_pose, offsets
